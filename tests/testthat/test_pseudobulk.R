...
test_that("raw-count pseudobulk sum then logCPM works and empty pools are rejected", {
  ...
  expect_equal(as.numeric(pb[, "p1"]), c(4, 0))
...
test_that("rc_check_pseudobulk_mapping verifies active pool sums", {
  skip_if_not_installed("Matrix")
  counts <- Matrix::Matrix(c(1, 0, 3, 0, 0, 0), nrow = 2, ncol = 3, sparse = TRUE)
  rownames(counts) <- c("g1", "g2"); colnames(counts) <- paste0("c", 1:3)
  pool_map <- data.frame(pool_id = c("p1", "p1", NA), cell_id = colnames(counts), skipped = c(FALSE, FALSE, TRUE))
  pb <- rc_pseudobulk_counts(counts, pool_map)
  expect_true(rc_check_pseudobulk_mapping(counts, pool_map, pb))
})
