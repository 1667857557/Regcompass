...
test_that("selected cores and annotation-related reactions are all targets", {
  ...
  expect_false("model_only" %in% colnames(definition$expanded_scoring_targets))
...
test_that("gene selection resolves only previous core reactions", {
  gem <- target_union_test_gem()
...
test_that("target cache points to the exact previous union GEM file", {
  gem <- target_union_test_gem()
...
test_that("second LP pass evaluates targets directly on the cached union", {
