...
test_that("stage-parallel CORDA2 preserves serial mathematical state", {
  ...
  parallel <- RegCompassR:::.rc_corda_build_three_stage_core(
    split = case$split, classes = case$classes, options = case$options,
    solver = "highs", time_limit = 30
  )
...
test_that("CORDA2 progress records completed and remaining targets", {
  ...
  context <- RegCompassR:::.rc_layer2_task_context(
    cell_type = "epithelial", medium_scenario = "plasma", route = "corda2"
  )
  for (i in 1:3) {
    RegCompassR:::.rc_corda_stage_mark_progress(
      progress_dir = progress_dir, index = i, total = 3L,
      stage = "corda2_step1_HC_dependencies", context = context,
      target = paste0("R", i)
    )
  }
