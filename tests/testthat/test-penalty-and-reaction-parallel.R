...
test_that("condition penalty entry uses padj, corr and estimate gates", {
  coefficient <- data.frame(
    estimate = c(0.01, 0.0099, -0.02, 0.02, 0.02),
    padj = c(0.049, 0.001, 0.049, 0.05, 0.001),
    corr = c(0.1, 0.5, -0.1, 0.5, 0.099),
    estimable = TRUE,
...
test_that("standard Pando uses the same corr and estimate gates", {
  edges <- data.frame(
    estimate = c(0.01, 0.009, 0.02, 0.02),
    padj = c(0.01, 0.01, 0.05, 0.01),
    corr = c(0.1, 0.5, 0.5, 0.099),
    stringsAsFactors = FALSE
  )
  selected <- RegCompassR:::.rc_filter_standard_pando_edges(edges)
  expect_equal(nrow(selected), 1L)
  expect_equal(selected$estimate, 0.01)
  expect_identical(
