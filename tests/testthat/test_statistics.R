test_that("rc_sample_aggregate uses sample-condition-celltype medians", {
  ...
  out <- rc_sample_aggregate(score_mat, pool_meta, condition_col = "condition")
...
test_that("rc_sample_summary reports sample-level flags", {
  ...
  pool_meta <- data.frame(pool_id = c("P1", "P2"), sample_id = "S1", condition = "ctrl", cell_type = "T", n_cells = c(40, 20), low_power_pool = c(FALSE, TRUE))
  out <- rc_sample_summary(score_mat, pool_meta, condition_col = "condition")
