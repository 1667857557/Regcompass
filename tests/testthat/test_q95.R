...
test_that("rc_q95_shrink uses reaction-specific finite n within strata", {
  ...
  colnames(C_raw) <- paste0("p", 1:4)
  unit_meta <- data.frame(pool_id = colnames(C_raw), cell_type = c("A", "A", "B", "B"))
  out <- rc_q95_shrink(C_raw, unit_meta = unit_meta, stratum_col = "cell_type")
