test_that("v1.8.0 workflow preserves canonical GPR architecture", {
  ...
  layer1_text <- paste(deparse(body(.rc_build_condition_pooled_layer1)), collapse = "\n")
...
test_that("full-library logCPM preserves matrix identifiers", {
  counts <- Matrix::Matrix(matrix(c(10, 5, 0, 5), nrow = 2, byrow = TRUE,
    dimnames = list(c("G1", "G2"), c("m1", "m2"))), sparse = TRUE)
  observed <- .rc_metacell_logcpm(counts, library_size = c(m1 = 100, m2 = 200))
...
test_that("condition comparison uses the primary penalty under shared vmax", {
  ...
  result <- list(
    penalty = matrix(c(4, 5, 1, 2), nrow = 1, dimnames = list(row_id, paste0("u", 1:4))),
    vmax = matrix(1, nrow = 1, ncol = 4, dimnames = list(row_id, paste0("u", 1:4))),
    params = list(omega = 1),
    unit_meta = data.frame(unit_id = paste0("u", 1:4), condition = c("A", "A", "B", "B"), cell_type = "T"))
