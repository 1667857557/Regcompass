  ...
  rc_solve_lp <- function(obj, A, lhs, rhs, lb, ub,
    ...
    answer <- highs::highs_solve(
      ...
      control = highs::highs_control(
        log_to_console = FALSE, output_flag = FALSE, threads = 1L,
        solver = "simplex", primal_feasibility_tolerance = 1e-7,
...
evalq({
  .rc_corda_stage_param <- function(n_targets) {
    n_targets <- as.integer(n_targets[[1L]])
    workers <- min(2L, n_targets)
    if (workers <= 1L) return(FALSE)
    BiocParallel::SnowParam(
      workers = workers, type = "SOCK", progressbar = FALSE
    )
  }
  .rc_corda_stage_parallel_requested <- function() FALSE
}, envir = e)
...
evalq(.rc_corda_stage_parallel_requested <- function() TRUE, envir = e)
...
stopifnot(
  ...
  identical(parallel_result$parallel_execution_policy,
            "stage_barrier_parallel_targets_deterministic_ordered_reduce")
)

cat("CORDA2 SnowParam stage-parallel serial-equivalence check passed\n")
