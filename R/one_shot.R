...
#' per broad cell type and keeps final metacells condition-pure. Optional raw
#' ATAC fragments are aggregated to the final metacell membership and recounted
#' before metacell TF-IDF. Layer 2 uses original MATLAB CORDA2 by default;
#' FASTCORE and full-GEM scoring remain explicit supplementary routes through
#' arguments passed in `...`.
#'
...
#'   omitted.
#' @param fragment_files Optional Stage 2 fragment input. Use `NULL`/`FALSE`, a
#'   fragment path (or named path vector), or a data frame with
...
#'   SOCK/Snow workers on Windows and Multicore workers on Linux/macOS. The
#'   effective cap is `min(workers, max(1, detected logical CPUs - 2))`, and
#'   each dispatch shrinks further to its number of independent tasks.
