# Layer 2 shared-structure validation and condition-specific primary scoring.
...
.rc_layer2_comparison_table <- function(
  ...
  support_fraction <- reaction_unit_matrix(
    layer1$reaction_condition_support_fraction
  )
  data.frame(
    ...
    metacell_id = unit,
    penalty_condition_oof = primary,
    penalty_rna_only = index_matrix(layer2$penalty_rna_only),
    penalty_grn_increment = index_matrix(layer2$penalty_grn_increment),
    penalty_per_target_flux = normalized,
    vmax = vmax,
    projection_oof_available = is.finite(primary),
    condition_support_fraction = support_fraction,
...
#'
#' With `model_mode = "meta_module_gem"`, this stage constructs one union GEM
#' per medium and reuses the exact model for every condition and metacell. The
#' condition-specific OOF route is the only primary multiome penalty. RNA-only
#' scoring is retained as an interpretation control. Depth matching, common-depth
#' restriction, alpha sensitivity, zero-support sensitivity, and link-saturation
#' propagation are not calculated or persisted.
#'
#' @export
rc_regcompass_step_layer2 <- function(
  ...
  answer$penalty_condition_oof <- answer$penalty
  answer$penalty_rna_only <- rna_only$penalty
  answer$penalty_grn_increment <- answer$penalty - answer$penalty_rna_only
  answer$score_condition_oof_display_only <- answer$score
  answer$score_rna_only_display_only <- rna_only$score
  answer$comparison_contract <- list(
    primary = "penalty_condition_oof",
    control = "penalty_rna_only",
    grn_increment = "penalty_condition_oof - penalty_rna_only",
