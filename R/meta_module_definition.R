...
rc_map_meta_module_core_reactions <- function(gene_nodes, gpr_table) {
  ...
  if (!is.data.frame(gpr_table) ||
    ...
    stop(
      paste(
        "`gpr_table` must contain reaction_id, and_group_id and gene;",
        "treating every gene as an isoenzyme would misclassify required",
        "subunits."
      ),
      call. = FALSE
    )
...
.rc_expand_meta_module_reactions_core <- function(
  ...
  maps$subsystem <- .rc_clean_meta_module_map(
    maps$subsystem, "subsystem_id"
  )
  maps$kegg <- .rc_clean_meta_module_map(maps$kegg, "kegg_id")
  maps$reactome <- .rc_clean_meta_module_map(
    maps$reactome, "reactome_id"
  )
  ...
  for (key in names(groups)) {
    ...
    database_seed <- included
    kegg_ids <- unique(maps$kegg$kegg_id[
      maps$kegg$reaction_id %in% database_seed
    ])
    reactome_ids <- unique(maps$reactome$reactome_id[
      maps$reactome$reaction_id %in% database_seed
    ])
    ...
    master_ids <- unique(maps$rhea_master$rhea_master_id[
      maps$rhea_master$reaction_id %in% rhea_seed
    ])
