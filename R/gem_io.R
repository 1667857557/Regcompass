...
rc_validate_model_info <- function(model_info) {
  required <- c("source_model", "model_name", "model_version", "source_release",
  ...
  miss <- required[!vapply(required, function(x) !is.null(model_info[[x]]) && length(model_info[[x]]) > 0L && !all(is.na(model_info[[x]])), logical(1))]
  if (length(miss)) stop("`model_info` is incomplete: ", paste(miss, collapse = ", "), call. = FALSE)
  invisible(TRUE)
}
