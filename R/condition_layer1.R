...
.rc_condition_increment_summary <- function(
  ...
  increment <- as.matrix(microcompass$penalty_unique_increment)
  vmax <- as.matrix(microcompass$vmax)
  if (!identical(dimnames(increment), dimnames(vmax))) {
    stop("Unique-increment penalty and vmax matrices are not aligned.",
         call. = FALSE)
  }
  omega <- microcompass$params$omega %||% 0.95
  ...
  if (!setequal(unit_id, colnames(normalized))) {
    stop("Unique-increment units and metadata differ.", call. = FALSE)
  }
  ...
  rows <- lapply(seq_len(nrow(strata)), function(i) {
    ...
    data.frame(
      ...
      median_unique_increment_per_required_target_flux = value,
