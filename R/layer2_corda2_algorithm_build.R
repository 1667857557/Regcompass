...
.rc_corda_build_three_stage_core <- function(
  ...
  if (!use_stage_parallel) {
    ...
    for (i in seq_along(stage1_hc)) {
      ...
      hc_present <- union(
        hc_present, active[directional_class[active] == "HC"]
      )
  ...
  # original Step 1 -> Step 2.1 -> Step 2.2 -> Step 3 barriers. Each target in a
  # stage sees the same immutable pre-stage state; results are restored to the
  # original target order before any confidence or dependency state changes.
  directional_class <- .rc_corda2_directional_class(split, classes)
  initial_directional_class <- directional_class
  hc <- names(directional_class)[directional_class == "HC"]
  mc <- names(directional_class)[directional_class == "MC"]
