...
rc_parallel_lapply <- function(X, FUN, BPPARAM = NULL, ...) {
  ...
  worker_fun <- function(x) {
    ...
    dispatch_env <- c(
      ...
      REGCOMPASS_DISPATCH_BACKEND = as.character(global_backend)
    )
  ...
  if (length(X) == 1L) {
    # Run the single cell-type controller in the current R process.  Its
    # REGCOMPASS_TASK_WORKERS budget is the full global cap, allowing Pando to
    # parallelize targets without wasting one separate outer worker pool.
    return(lapply(X, worker_fun))
  }

  outer_param <- .rc_parallel_param_for_tasks(BPPARAM, length(X))
