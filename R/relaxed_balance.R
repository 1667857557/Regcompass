...
#' @export
rc_run_relaxed_balance_lp <- function(microgem, penalties, target_reaction, target_direction = "forward", omega = 0.95, target_min_if_strict_infeasible = 1e-6, slack_penalty = NULL, slack_penalty_mode = c("relative", "absolute"), slack_penalty_multiplier = 5, metabolite_slack_weights = NULL, solver = "highs", time_limit = 60) {
  ...
  lhs <- rhs <- rep(0, m)
  target_min_used <- if (isTRUE(vmax$feasible)) omega * vmax$vmax else target_min_if_strict_infeasible
  j <- match(target_reaction, colnames(gv$S)); at <- rep(0, 2*n + 2*m)
  if (target_direction == "reverse") { at[j] <- -1; at[n+j] <- 1 } else { at[j] <- 1; at[n+j] <- -1 }
  Aeq <- rbind(Aeq, at); lhs <- c(lhs, target_min_used); rhs <- c(rhs, Inf)
