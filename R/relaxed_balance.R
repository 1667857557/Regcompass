...
#' @export
rc_run_relaxed_balance_lp <- function(microgem, penalties, target_reaction, target_direction = "forward", omega = 0.95, slack_penalty = 100, metabolite_slack_weights = NULL, solver = "highs", time_limit = 60) {
  ...
  data.frame(target_reaction=target_reaction, target_direction=target_direction, strict_feasible=isTRUE(vmax$feasible), relaxed_feasible=identical(ans$status,"optimal"), total_slack=total, top_slack_metabolites=paste(utils::head(rownames(gv$S)[order(sp+sm, decreasing=TRUE)],5), collapse=";"), cofactor_slack_used=sum((sp+sm)[role %in% TRUE], na.rm=TRUE), boundary_slack_used=NA_real_, slack_penalty=slack_penalty, solver_status=ans$status, stringsAsFactors=FALSE)
}
