# Exact serial reference for the original MATLAB CORDA2 state machine.
#
# This is the pre-stage-parallel implementation kept intact so `parallel = FALSE`
# and one-worker execution retain the original persistent-engine behavior.
...
.rc_corda_stage_parallel_requested <- function() {
  ...
  available <- get0("rc_available_workers", mode = "function", inherits = TRUE)
  if (!is.function(available) ||
      !requireNamespace("BiocParallel", quietly = TRUE)) return(FALSE)
  as.integer(available(default = 1L)) > 1L
}

.rc_corda_build_three_stage_serial_core <- function(
