...
#' Export a metacell- or sample-level long table from a reaction-by-column matrix
#' @export
...
#' Sample-level long summary with meta-cell/cell-count diagnostics
rc_sample_summary <- function(score_mat, unit_meta, sample_col = "sample_id", celltype_col = "cell_type", condition_col = NULL) {
  score_mat <- as.matrix(score_mat)
  ...
  unit_meta <- unit_meta[unit_meta$pool_id %in% colnames(score_mat), , drop = FALSE]
  group_cols <- c(sample_col, condition_col, celltype_col)
  group_cols <- group_cols[!is.null(group_cols) & !is.na(group_cols)]
  keys <- interaction(unit_meta[, group_cols, drop = FALSE], drop = TRUE, sep = "|")
  pieces <- lapply(split(unit_meta, keys), function(pm) {
    vals <- score_mat[, pm$pool_id, drop = FALSE]
    data.frame(
      group_id = unique(keys[match(pm$pool_id, unit_meta$pool_id)])[1],
