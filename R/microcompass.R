...
.rc_validate_shared_medium <- function(medium_scenarios) {
  ...
  if ("condition" %in% colnames(medium_scenarios)) {
    ...
    if (length(condition)) {
      stop(
        "Shared-GEM scoring requires condition-invariant medium constraints; remove condition-specific medium rows.",
        call. = FALSE
      )
    }
  }
...
.rc_build_global_meta_module_gem_cache <- function(
  ...
  if (!is.data.frame(reaction_membership) ||
    ...
    stop(
      "`reaction_membership` must contain sample_id, module_id and reaction_id.",
      call. = FALSE
    )
  }
  if (!is.data.frame(core_reactions) ||
    ...
    stop(
      "`core_reactions` must contain sample_id, module_id and reaction_id.",
      call. = FALSE
    )
  }
  ...
  if (!nrow(core_reactions)) {
    stop("No global core reactions remain for scoring.", call. = FALSE)
  }
  ...
  for (scenario in scenarios) {
    ...
    file <- file.path(
      cache_dir,
      paste0("global_meta_module__medium_", safe(scenario), ".rds")
    )
    saveRDS(model, file)
...
.rc_run_microcompass_engine <- function(
  ...
  if (identical(mode, "full_gem")) {
    ...
    model_cache <- .rc_build_global_meta_module_gem_cache(
      ...
      medium_scenarios = medium_scenarios,
      cache_dir = model_params$cache_dir %||%
        tempfile("RegCompassR_global_meta_module_cache_"),
      target_direction = target_direction,
      solver = solver,
    ...
    if (!length(model_cache)) {
      stop("No parent-feasible global meta-module targets were available.",
           call. = FALSE)
    }
  ...
  results <- unlist(
    lapply(grouped_results, `[[`, "results"),
    recursive = FALSE
  )
  ...
  lp_diagnostics <- do.call(
    rbind,
    lapply(grouped_results, `[[`, "diagnostics")
  )
  ...
  list(
    ...
    method = if (identical(mode, "full_gem")) {
      ...
      "microCOMPASS shared global meta-module-GEM directional LP"
    }
  )
}
