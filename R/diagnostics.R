#' Minimal meta-cell diagnostics for the simplified Layer 1 workflow
rc_unit_diagnostics <- function(unit_map,
                                rna_counts = NULL,
  ...
  missing_cols <- setdiff(c("pool_id", "cell_id"), colnames(unit_map))
  if (length(missing_cols) > 0L) stop("`unit_map` is missing required columns: ", paste(missing_cols, collapse = ", "), call. = FALSE)

  active_unit_map <- unit_map
  if ("skipped" %in% colnames(active_unit_map)) {
    active_unit_map <- active_unit_map[!active_unit_map$skipped, , drop = FALSE]
  }
  active_unit_map <- active_unit_map[!is.na(active_unit_map$pool_id), , drop = FALSE]

  if (!is.null(rna_counts)) {
    rc_validate_diagnostic_matrix(rna_counts, active_unit_map$cell_id, "rna_counts")
  }
  ...
  pool_ids <- unique(active_unit_map$pool_id)
  pieces <- rc_parallel_lapply(pool_ids, function(pid) {
    rows <- active_unit_map$pool_id == pid
    cells <- active_unit_map$cell_id[rows]
    one <- active_unit_map[rows, , drop = FALSE]

    data.frame(
      pool_id = pid,
      sample_id = rc_unit_unique_value(one, sample_col),
      cell_type = rc_unit_unique_value(one, celltype_col),
      condition = rc_unit_unique_value(one, condition_col),
      ...
      pool_seed = rc_unit_unique_value(one, "pool_seed"),
      state_source = rc_unit_unique_value(one, "state_source"),
      state_resolution = rc_unit_unique_value(one, "state_resolution"),
...
rc_validate_diagnostic_matrix <- function(mat, cells, name) {
  ...
  if (length(missing_cells) > 0L) stop("Some unit_map cell IDs are absent from `", name, "`: ", paste(utils::head(missing_cells), collapse = ", "), call. = FALSE)
  invisible(TRUE)
}

rc_unit_unique_value <- function(df, col) {
