...
For cell types with at least two retained conditions, RegCompass first parallelizes pooled-background and condition × cell-type candidate discovery, freezes one exact edge dictionary per cell type after a strict barrier, and then parallelizes condition × cell-type fixed-dictionary GLMs. This tutorial uses `tf_cor = 0.1` and `peak_cor = 0` for that candidate screen. Each atomic Pando task disables nested target-level parallelism. If only one effective condition is retained, standard Pando is parallelized across broad cell types and treats the requested `tf_cor` as a biological floor, automatically raising it when necessary to the two-sided Pearson-correlation critical value for that cell type's actual cell count (`alpha = 0.05`).
