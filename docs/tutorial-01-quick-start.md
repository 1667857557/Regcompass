...
Pando `estimate` values are still fitted and stored, but their absolute magnitudes are not used as Layer-1 penalty weights. After the existing estimability/fit-status/BH gate, RegCompass keeps only `sign(estimate)`. For each fitted TF–peak pair, the paired-cell product `TF * ATAC` is robustly self-scaled across all fitted cells of the same broad cell type and bounded with `tanh`. For each target and condition, signed edge activities are averaged across that target's active Pando edges, preventing a larger significant-edge count from creating a larger regulatory score by itself; these cell scores are then averaged by the exact Stage-2 SuperCell membership. The existing target-level projection calibration, RNA support, GPR aggregation, COMPASS reaction cost and Layer-2 LP are unchanged. No new public parameter is required for this sign-only projection.
