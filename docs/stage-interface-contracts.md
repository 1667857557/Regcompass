...
The condition-metacell cache contract covers ordered cell IDs with condition/cell-type assignments, RNA and ATAC assay fingerprints, the fixed SuperCell2 construction label, reductions and dimensions, `gamma`, seed, and metacell thresholds. When checkpoint files already exist and `overwrite = FALSE`, the requested contract must be identical. A missing or changed contract requires `metacell_args = list(overwrite = TRUE)`.
