...
| `rc_regcompass_step_target_union()` | matching Stage 3-5 objects from a union-GEM run | `regcompass_target_union_step` | selected anchors must be previous core targets; expanded targets must exist in the original cached union GEM |
...
- a target-union expansion requests a reaction absent from the original union;
- a cached union-model file is missing or has changed provenance.

These checks prevent numerically valid but biologically unrelated stage objects from being combined.
