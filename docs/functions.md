...
- `rc_regcompass_step_grn()`: normalize single-cell RNA, align assay data by cell ID, compute cell-type-shared ATAC TF-IDF across conditions, and fit one Pando model per condition × cell type. Default `peak_cor = 0.01`. Pando may be installed from GitHub or a local source archive; required API functions are validated.
- `rc_regcompass_step_metacells()`: build SuperCell2 metacells stratified only by condition. Default `gamma = 75`. Cell type is assigned afterwards from the dominant member-cell label. Exact dominant-label ties are rejected. The stage writes final root-level metadata, membership, composition, and summary tables.
...
- `rc_regcompass_step_layer2()`: preflight the selected LP solver, run directional COMPASS-like minimum-penalty scoring, and maintain the persistent model cache. The default HiGHS solver is a required dependency.
