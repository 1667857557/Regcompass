...
The default Stage 1 thresholds are `tf_cor = 0.05`, `peak_cor = 0.05`, and `padj_threshold = 0.05`; these are defaults, not fixed constants, and may be overridden through `pando_args$pando_infer_args`. BH adjustment remains the canonical default. For Condition Pando, `padj_threshold` controls the preliminary ridge-Wald/BH dictionary screen and condition-specific support mask only. The selected common dictionary is then jointly refit for final condition effects with coefficient and contrast P-value calculation disabled. For Standard Pando, `padj_threshold` remains the one-stage final active-edge gate. Known parameters that belong only to the other Pando route are ignored for the incompatible route and recorded in routing diagnostics, while genuinely unknown parameter names still fail validation. Standard Pando defaults to `method = "ridge"`, the K=1 specialization of the same ridge solver; `method = "glm"` remains an explicit compatibility option.
