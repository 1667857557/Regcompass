...
Metacell-level condition tests describe within-dataset separation and are not automatically biological-replicate-level treatment inference. See [Condition-associated reaction statistics](condition-reaction-statistics.md) for the score definition, reaction annotations, reaction-level RNA-versus-multiome evidence, gene-driven selection, multiple-testing scope, plotting examples, and inference boundary.
